...
export const fetchAspirations = async (): Promise<Aspirations[]> => {
    const cacheKey = 'aspirations:all';

    // Try to get from cache first
    const cached = cacheManager.get<Aspirations[]>(cacheKey);
    if (cached) {
        console.log('[Aspirations] Returning cached aspirations');
        return cached;
    }
    ...
    try {
        ...
        // Cache the aspirations
        cacheManager.set(cacheKey, aspirationData, CACHE_TTL.ASPIRATIONS);
...
export const CreateAspiration = async (
    ...
    try {
        ...
        // Invalidate aspirations cache since we created a new aspiration
        cacheManager.invalidate('aspirations:*');
...
export const GetAspirationById = async (id: number): Promise<Aspirations> => {
    const cacheKey = `aspirations:id:${id}`;

    // Try to get from cache first
    const cached = cacheManager.get<Aspirations>(cacheKey);
    if (cached) {
        console.log(`[Aspirations] Returning cached aspiration: ${id}`);
        return cached;
    }
    ...
    try {
        ...
        // Cache the aspiration
        cacheManager.set(cacheKey, aspiration, CACHE_TTL.ASPIRATIONS);
...
export const AdminReplyAspiration = async (
    ...
    try {
        ...
        // Invalidate aspirations cache since we added a reply
        cacheManager.invalidate('aspirations:*');
...
export const DeleteAspiration = async (id: number | string, accessToken: string): Promise<{ success: boolean; message: string }> => {
    try {
        ...
        // Invalidate aspirations cache since we deleted an aspiration
        cacheManager.invalidate('aspirations:*');
