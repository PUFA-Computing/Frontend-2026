...
export default async function EventsPage() {
  ...
  // Filter events related to PUMA IS (using case-insensitive comparison)
  const pumaIsEvents = events.filter((event) => {
    // Check for both "PUMA IS" and "PUMA-IS" formats and normalize case
    const org = event.organization?.toLowerCase() || "";
    return org === "puma is" || org === "puma-is" || org === "pumais";
  })
      ...
      <section className="container mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        {upcomingEvents.length > 0 && upcomingEvents[0] ? (
              ...
              <div className="flex flex-col justify-between p-6 md:col-span-3 lg:col-span-2">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {upcomingEvents[0]?.slug && (
                    <Link href={`/events/${upcomingEvents[0].slug}`} className="mt-4 inline-flex items-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700">
                      View Details
          ...
          <div className="rounded-xl bg-white p-8 text-center shadow-md">
            <Calendar className="mx-auto mb-4 h-12 w-12 text-blue-200" />
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute right-3 top-3">
                        <span className="rounded-full bg-blue-600 px-2 py-1 text-xs font-medium text-white shadow-md">
                          {new Date() < event.start_date ? "Upcoming" : 
                           (new Date() >= event.start_date && new Date() <= event.end_date) ? "Ongoing" : "Ended"}
                        </span>
                      </div>
                    </div>
                    
                    <div className="p-4">
                      <h3 className="mb-2 text-lg font-bold text-gray-900 group-hover:text-blue-600">{event.title}</h3>
