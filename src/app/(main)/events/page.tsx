...
import { CalendarDays, Clock, Sparkles, Calendar, MapPin, Users, ChevronRight, CalendarX } from "lucide-react"
...
export default async function EventsPage() {
  ...
  const sortedEvents = hasEvents
    ? [...events!].sort((a, b) => b.start_date.getTime() - a.start_date.getTime())
    : []

  // Get upcoming events (first 3)
  const upcomingEvents = hasEvents
    ? sortedEvents.filter((event) => event.end_date.getTime() >= today.getTime()).slice(0, 3)
    : []

  // Get all events for timeline, sorted chronologically (newest first)
  const allEvents = hasEvents
    ? [...events!].sort((a, b) => b.start_date.getTime() - a.start_date.getTime())
    : []

  const truncateDescription = (description: string | null | undefined, length: number = 100) => {
    ...
    <div className="min-h-screen">
      ...
      <section id="upcoming" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 md:px-8 lg:px-10">
        ...
        {upcomingEvents.length > 0 ? (
          ...
          <div className="rounded-2xl border border-[#B8841E]/15 bg-[#FAF5E8]/60 p-12 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-amber-100/80 border border-amber-200">
              <CalendarX className="h-8 w-8 text-amber-500" />
            </div>
            <h3 className="font-display italic text-xl text-[#0D1B3E] mb-2">No Upcoming Events</h3>
            <p className="font-serif text-[#1A1A2E]/60 text-sm max-w-sm mx-auto">
              There are currently no upcoming events scheduled. Check back soon for new activities!
            </p>
          </div>
        )}
      </section>
      ...
      <section className="mx-auto w-full py-12 lg:max-w-7xl">
        ...
        {allEvents.length > 0 ? (
          ...
          <div className="px-3 sm:px-4">
            <div className="rounded-2xl border border-[#B8841E]/15 bg-[#FAF5E8]/60 p-16 text-center">
              <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-[#B8841E]/10 border border-[#B8841E]/20">
                <CalendarX className="h-10 w-10 text-[#B8841E]/50" />
              </div>
              <h3 className="font-display italic text-2xl text-[#0D1B3E] mb-3">No Events Available</h3>
              ...
              <p className="font-serif text-[#1A1A2E]/60 max-w-sm mx-auto leading-relaxed">
                There are currently no events scheduled. Please check back later for exciting upcoming activities.
              </p>
            </div>
          </div>
        )}
